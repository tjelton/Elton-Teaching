...
> ⚠️ Small wrinkle: the *primitive* `int` is not a `Number` — primitives aren't objects. But the wrapper class `Integer` is. When you write `list.add(42)`, Java auto-boxes the `int` into an `Integer`, so in generics-land "int" effectively means `Integer`.

#### `? extends Number` — read-only

Read this as: *"a list of **some specific** subtype of `Number`, but I don't know which one."*

The actual list might really be a `List<Integer>`, a `List<Double>`, a `List<Long>`, or just a `List<Number>`. The compiler doesn't know which.
...
An `Integer` fits everywhere up the chain — it's-a `Integer`, is-a `Number`, is-an `Object`:
...
**Reading is (mostly) useless ✗**

```java
List<? super Integer> sink = new ArrayList<Object>();
Integer i = sink.get(0);   // ✗ might be a String — compile error
Object  o = sink.get(0);   // ✓ everything is an Object
```

The only type the compiler can guarantee every element belongs to is `Object`, because the list might really be a `List<Object>` containing anything.
...
- If `? super` allowed reads as the bound → you could pull a `String` out of a `List<Object>` and assign it to an `Integer`.
